...
Registry、匹配、依赖、冲突、合并和阶段转换用确定性测试验证。Broker 与 Kimi 的交界先用假适配器验证契约，再用固定版本 SDK 和受控 MCP 做真实集成测试。桌面端需要覆盖项目切换、权限、断连、产物查看与应用重启。Viewer 按格式验证大文件、配套输入缺失、格式错误、跨项目引用和显示缓存失效；CAD/Godot 的验收聚焦关键产物预览。发行物按目标操作系统分别验证，不用单一平台构建结果推断其他平台可用。

Domain Task bench 应调用独立 CLI，按任务指定项目目录与 Domain，保存 JSONL 事件、最终状态和 Broker 披露 Trace。CLI 与桌面端使用同一 Harness Core，不依赖 Electron、Viewer 或渲染进程；同一任务的评测结果应能关联运行配置和产物输入。
