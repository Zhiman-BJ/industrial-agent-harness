...
for (const file of [entry, skill, mcp]) if (!fs.existsSync(file)) throw Error(`Incomplete headless package: ${file}`);
fs.writeFileSync(path.join(target, 'industrial-harness.cjs'), '#!/usr/bin/env node\nvoid require("./src/main.cjs").main();\n', {mode: 0o755});
fs.writeFileSync(path.join(target, 'HEADLESS-README.txt'), 'Industrial Agent Harness headless package\n\nRun with Node.js 22 or newer:\n  node industrial-harness.cjs run --project-dir DIR --domain DOMAIN --task TEXT --scope-only\n  node industrial-harness.cjs bench --suite FILE --output-dir DIR\n\nAgent execution additionally needs a Kimi CLI executable and model API key in the environment. This package contains the Harness CLI, Broker, Skill files, MCP registry, and Kimi SDK integration. It does not bundle the Kimi CLI or industrial executables. The current MCP registry has no default servers.\n');
